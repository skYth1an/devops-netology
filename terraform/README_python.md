...
| Вопрос  | Ответ |
| ------------- | ------------- |
| Какое значение будет присвоено переменной `c`?  | никакое,ошибка при складывании числа и строки  |
...
bash_command = ["cd C:/Users/ilebe/OneDrive/Рабочий стол/Git-Repo/github", "git status"]
result_os = os.popen(' && '.join(bash_command)).read()
for result in result_os.split('\n'):
    if result.find('modified') != -1:
        prepare_result = result.replace('\tmodified:   ', '')
...
path_to_git = input("Введите абсолютный путь к директории Git: ")
